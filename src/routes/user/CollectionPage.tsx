  ...
  const loadCollection = async () => {
    try {
      ...
      const collectionArtworks: CollectionArtwork[] = (data || []).map(item => ({
        ...
        artist: {
          name: item.artworks.profiles?.full_name || 'Unknown Artist',
          slug: item.artworks.profiles?.slug || item.artworks.id
        },
        primaryImageUrl: item.artworks.primary_image_url || '',
        price: item.artworks.price || 0,
        currency: item.artworks.currency || 'ZAR',
        medium: item.artworks.medium || '',
        year: item.artworks.year || new Date().getFullYear(),
